...
def emscript_wasm_backend(infile, settings, outfile, libraries=None, compiler_engine=None,
  ...
  if shared.Settings.WASM_BACKEND == 2:
    ...
    pre = ''
    post = ''
    metadata = extract_metadata_wasm(wasm_file)
    if DEBUG: logging.debug("meta raw: %s", metadata)
  else:
    wast = build_wasm_s2wasm(temp_files, infile, outfile, settings, DEBUG)
    # Integrate info from backend
    metadata_raw = read_metadata(wast)
    if DEBUG: logging.debug("meta raw: %s", metadata_raw)
  ...
  metadata = create_metadata_wasm(metadata, wast)
...
def wasm_dis(wasm_file, wast_file):
  """Disassemle a wasm binary, converting to a wast."""
  wasm_dis_args = [os.path.join(shared.Settings.BINARYEN_ROOT, 'bin',
    'wasm-dis'), wasm_file, '-o', wast_file]
  logging.debug('emscript: wasm-dis: ' + ' '.join(wasm_dis_args))
  shared.check_call(wasm_dis_args)


def extract_metadata_wasm(wasm_file):
  """Extract metadata from wasm binary.

  This is used in the lld path to extract the metadata which is otherwise
  generated by s2wasm and stuffed into the comment in the wast file.
  """
  metadata = {}
  readobj_args = [shared.LLVM_READOBJ, '-s', wasm_file]
  output = shared.check_execute(readobj_args)
  for line in output.splitlines():
    line = line.strip()
    if line.startswith('DataSize:'):
      metadata['staticBump'] = int(line.split(':')[1])
  metadata['asmConsts'] = {}
  return metadata
...
def create_metadata_wasm(metadata, wast):
  metadata_defaults = {
    'declares': [],
  ...
  for k, v in metadata_defaults.iteritems():
...
def create_lld_args(temp_o, output):
  ...
  args += ['--global-base=%d' % shared.Settings.GLOBAL_BASE]
  args += ['--initial-memory=%d' % shared.Settings.TOTAL_MEMORY]
  args += [libc_rt_lib, compiler_rt_lib]
  args += ['-o', output]
  return args
